...
class TestProjectEntry:
    ...
    def test_from_dict_with_all_fields(self):
        """Test creating a ProjectEntry from a complete dictionary."""
        data = {
    ...
    def test_from_dict_missing_added(self):
        """Test creating a ProjectEntry with missing added field."""
    ...
    def test_to_dict(self):
        """Test converting a ProjectEntry to a dictionary."""
        entry = ProjectEntry(
            path="/home/user/project",
            added="2025-01-15T10:30:00+00:00",
        )
...
class TestProjectRegistry:
    ...
    def test_path_custom(self, tmp_path: Path):
        ...
        registry = ProjectRegistry(custom_path)
        assert registry.path == custom_path

    def test_load_nonexistent_file(self, temp_registry: ProjectRegistry):
    ...
    def test_save_writes_json(self, temp_registry: ProjectRegistry, temp_project: Path):
    ...
    def test_register_adds_project(self, temp_registry: ProjectRegistry, temp_project: Path):
    ...
    def test_register_duplicate(self, temp_registry: ProjectRegistry, temp_project: Path):
    ...
    def test_unregister_removes_project(self, temp_registry: ProjectRegistry, temp_project: Path):
    ...
    def test_unregister_not_found(self, temp_registry: ProjectRegistry, temp_project: Path):
        """Test unregistering a project that's not registered."""
        result = temp_registry.unregister(temp_project)
        assert result is False

    def test_is_registered_true(self, temp_registry: ProjectRegistry, temp_project: Path):
        """Test is_registered returns True for registered project."""
        temp_registry.register(temp_project)
        assert temp_registry.is_registered(temp_project) is True

    def test_is_registered_false(self, temp_registry: ProjectRegistry, temp_project: Path):
        """Test is_registered returns False for unregistered project."""
        assert temp_registry.is_registered(temp_project) is False

    def test_get_entry_found(self, temp_registry: ProjectRegistry, temp_project: Path):
    ...
    def test_get_entry_not_found(self, temp_registry: ProjectRegistry, temp_project: Path):
        """Test get_entry returns None for unregistered project."""
        entry = temp_registry.get_entry(temp_project)
        assert entry is None

    def test_clear_removes_all(self, temp_registry: ProjectRegistry, tmp_path: Path):
...
class TestModuleFunctions:
    ...
    def test_register_project_current_dir(self, tmp_path: Path, monkeypatch):
        """Test register_project with current directory."""
    ...
    def test_register_project_with_path(self, tmp_path: Path):
        """Test register_project with explicit path."""
        import envdrift.agent.registry as registry_module
        ...
        success, message = register_project(project_dir)

        assert success is True
        assert "Registered" in message

    def test_register_project_nonexistent(self, tmp_path: Path):
        """Test register_project with nonexistent directory."""
        import envdrift.agent.registry as registry_module

        registry_path = tmp_path / ".envdrift" / "projects.json"
        registry_module._registry = ProjectRegistry(registry_path)

        success, message = register_project(tmp_path / "nonexistent")

        assert success is False
        assert "does not exist" in message

    def test_register_project_file_not_dir(self, tmp_path: Path):
        """Test register_project with a file instead of directory."""
        import envdrift.agent.registry as registry_module
        ...
        success, message = register_project(file_path)

        assert success is False
        assert "Not a directory" in message

    def test_unregister_project(self, tmp_path: Path):
        """Test unregister_project."""
        import envdrift.agent.registry as registry_module
        ...
        # Then unregister
        success, message = unregister_project(project_dir)

        assert success is True
        assert "Unregistered" in message

    def test_unregister_project_not_registered(self, tmp_path: Path):
        """Test unregister_project with unregistered project."""
        import envdrift.agent.registry as registry_module
    ...
    def test_list_projects(self, tmp_path: Path):
        ...
        register_project(project1)
        register_project(project2)
    ...
    def test_register_project_with_tilde(self, tmp_path: Path, monkeypatch):
        """Test register_project expands ~ to home directory."""
        import envdrift.agent.registry as registry_module
        ...
        # Mock home expansion
        monkeypatch.setattr(
            Path, "expanduser", lambda self: Path(str(self).replace("~", str(tmp_path)))
        )

        # This should work with the ~ path
        success, _message = register_project(str(project_dir))
        assert success is True
